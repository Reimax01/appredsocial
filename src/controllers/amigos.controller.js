...
amigos.registro = async(req, res) => {
    ...
    const existeRegistroAmigo = await conexion.query(`SELECT * from amigos where amigos.id_amigo = ${id_amigo} and amigos.id_usuario = ${id_usuario}`);
    console.log(existeRegistroAmigo);
    if (existeRegistroAmigo.rowCount === 0) {
...
amigos.aceptar_amistad = async (req, res) => {
    ...
    await conexion.query(query);
    let query2 = `UPDATE amigos SET es_aceptada='true' WHERE id_amigo = ${id_usuario} and id_usuario = ${id}`;
...
amigos.getSolicitudAmigos = async (req, res) => {
    const id = req.params.id_usuario;
    const response = await conexion.query(`SELECT es_aceptada, fecha_emision_amigo,es_seguido, es_bloqueado, amigos.id_usuario,amigos.id_amigo,usuario.nom_usuario, usuario.imagen_usuario 
from amigos,usuario where amigos.id_amigo = usuario.id_usuario and amigos.id_amigo = ${id} and es_aceptada = false`);
    res.status(200).json(response.rows);
}
